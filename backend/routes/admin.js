...
// Only the top-level admin (role = 'admin') can manage sub-admins and
// see other admin accounts. Sub-admins cannot create sub-admins or view
// the admin profile.
...
router.get('/users', async (req, res) => {
  try {
    ...
    // Sub-admins must not see admin / sub-admin accounts (admin profile hidden).
    const [me] = await pool.execute(
      'SELECT role FROM users WHERE id = ?',
      [req.user.userId]
    );
    const myRole = me[0]?.role || 'user';
    const roleFilter = myRole === 'admin' ? '' : 'AND u.role NOT IN (\'admin\', \'subadmin\')';

    const [users] = await pool.execute(
      `SELECT u.id, u.name, u.email, u.phone, u.password, u.car_number, u.is_admin, u.role,
...
// ── Own admin profile ─────────────────────────────────────────────────
// Returns the profile of the logged-in admin / sub-admin (their own).
router.get('/profile', async (req, res) => {
  try {
    const [rows] = await pool.execute(
      `SELECT id, name, email, phone, role, is_admin, created_at
         FROM users WHERE id = ?`,
      [req.user.userId]
    );
    ...
    res.json({
      profile: {
        ...
        is_admin: !!admin.is_admin || ['subadmin', 'admin'].includes(admin.role),
...
router.get('/sub-admins', requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.error('Admin sub-admins list error:', error);
...
router.post('/sub-admins', requireSuperAdmin, [
  ...
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO users
        (name, email, phone, password, membership_status, can_create_qr, can_hide_number, is_admin, role)
       VALUES (?, ?, ?, ?, 'active', TRUE, TRUE, TRUE, 'subadmin')`,
    ...
    console.error('Admin create sub-admin error:', error);
    res.status(500).json({ error: 'Failed to create sub-admin' });
  }
});

router.put('/sub-admins/:id', requireSuperAdmin, [
  ...
  try {
    ...
    if (!subAdminId) return res.status(400).json({ error: 'Invalid sub-admin ID' });

    const [rows] = await pool.execute(
      'SELECT id FROM users WHERE id = ? AND role = ?',
      [subAdminId, 'subadmin']
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Sub-admin not found' });
    ...
    if (req.body.password !== undefined) { updates.push('password = ?'); values.push(req.body.password); }
    ...
    console.error('Admin update sub-admin error:', error);
...
router.delete('/sub-admins/:id', requireSuperAdmin, async (req, res) => {
  try {
    ...
    await pool.execute('DELETE FROM users WHERE id = ?', [subAdminId]);
    res.json({ message: 'Sub-admin deleted successfully' });
  } catch (error) {
    console.error('Admin delete sub-admin error:', error);
