  ...
  try {
    ...
    try {
      const [result] = await pool.execute(
        `INSERT INTO notifications
         ...
         VALUES (?, ?, 'NOTIFICATION', 'SENT', NOW())`,
        [r.id, `KaroAlert: Your vehicle was scanned at ${now.toLocaleString()}. Someone may need you to move it. Kindly check the KaroAlert app.`]
      );
    ...
    if (io && ownerSocketId) {
      ...
      const notificationPayload = {
        ...
        sender_name: null,
        sender_mobile: null,
        message: `KaroAlert: Your vehicle was scanned at ${now.toLocaleString()}. Someone may need you to move it. Kindly check the KaroAlert app.`,
        notification_type: 'NOTIFICATION',
        status: 'SENT',
        sent_at: now,
        created_at: now,
...
router.post('/ring', async (req, res) => {
  try {
    ...
    const alertPayload = {
      ...
      message: 'Parking Alert: Your vehicle is blocking the way. Kindly move it or respond in the KaroAlert app.'
    };
