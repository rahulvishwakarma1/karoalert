...
// QR encodes a Play Store link (so Google Lens / camera scans without the app
// land on the app's Play Store page) plus the QR code id as a hidden param,
// which the KaroAlert in-app scanner extracts to trigger the scan flow.
const PLAY_STORE_BASE = 'https://play.google.com/store/apps/details?id=com.karoalert.app';
const qrContentUrl = (qrCodeId) => `${PLAY_STORE_BASE}&qr=${qrCodeId}`;
...
router.get('/my-qr', authenticateToken, async (req, res) => {
  try {
    ...
    const qrUrl = qrContentUrl(qr.qr_code_id);
    let qrImage;
    try {
...
router.post('/regenerate', authenticateToken, async (req, res) => {
  try {
    ...
    const qrUrl   = qrContentUrl(qrCodeId);
